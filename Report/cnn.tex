...
\label{sec:buildcnn}
%
As we discussed in the last section \ref{sec:introduction_keras}, Keras has a 
...
if it's a first layer it's connected to all the inputs. 
...
train. 
This issue appears often, it is one of reasons that deep neural networks can 
take a long time to learn. 
To understand this issue a bit more, let's consider the case of working with 
images. 
Assume that we have an 8 megapixel image and we want to learn something from 
this image. 
...
In a dense we connect the data from each pixel to each neuron so we end up with 
...
colors. 
One each for red, green, and blue channels of the image. 
...
And finally, to a fully connected layer and then to a second fully connected 
...
summarizing neighbouring information.
